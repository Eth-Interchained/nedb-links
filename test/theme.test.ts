...
test("theme cycle visits all four registers and wraps", () => {
  assert.deepEqual(THEME_ORDER, ["pro", "native", "v3", "mach"]);
  assert.equal(nextTheme("pro"), "native");
  assert.equal(nextTheme("native"), "v3");
  assert.equal(nextTheme("v3"), "mach");
  assert.equal(nextTheme("mach"), "pro", "cycle wraps back to pro");
